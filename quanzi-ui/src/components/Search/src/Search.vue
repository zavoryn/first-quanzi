...
<template>
  <!-- update by 元圈：class="-mb-15px" 用于降低和 ContentWrap 组件的底部距离，避免空隙过大 -->
  <Form
    :inline="inline"
    :is-col="isCol"
    :is-custom="false"
  ...
  >
    <template #action>
      <div v-if="layout === 'inline'">
        <!-- update by 元圈：去除搜索的 type="primary"，颜色变淡一点 -->
        <ElButton v-if="showSearch" @click="search">
          <Icon class="mr-5px" icon="ep:search" />
          {{ t('common.query') }}
        </ElButton>
        <!-- update by 元圈：将 icon="ep:refresh-right" 修改成 icon="ep:refresh" -->
