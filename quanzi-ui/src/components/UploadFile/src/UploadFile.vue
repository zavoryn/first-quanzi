<template>
  <div v-if="!disabled" class="upload-file">
    <el-upload
      ...
      :show-file-list="true"
      :accept="fileType.map(type => '.' + type).join(',')"
    ...
    >
      ...
      <template #file="row">
        <div class="flex items-center" v-if="!returnList">
...
const props = defineProps({
  ...
  returnList: propTypes.bool.def(false),
})
...
const emitUpdateModelValue = () => {
  ...
  // 情况2：逗号分隔的字符串
  if (!props.returnList && (props.limit === 1 || isString(props.modelValue))) {
    result = result.join(',')
  }
