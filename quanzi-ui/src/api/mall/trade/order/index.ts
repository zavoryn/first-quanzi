...
export const exportOrder = async (params) => {
  return await request.download({ url: '/trade/order/export', params })
}
//同步订单
export const syncTechProductOrder= () => {
  return request.get({ url: '/trade/order/syncTechProductOrder' })
}
//同步分类
export const syncTechCategory= () => {
  return request.get({ url: '/trade/order/syncTechCategory' })
}
// 查询交易订单统计
