package cn.metast.tuoke.framework.websocket.core.sender;
...
import cn.metast.tuoke.framework.websocket.core.message.JsonWebSocketMessage;
import cn.metast.tuoke.framework.websocket.core.session.WebSocketSessionManager;
import cn.metast.tuoke.framework.websocket.core.util.WebSocketFrameworkUtils;
...
public abstract class AbstractWebSocketMessageSender implements WebSocketMessageSender {
    ...
    public List<Long> getSessionList() {
        List<WebSocketSession> sessions = (List<WebSocketSession>) sessionManager.getSessionList(UserTypeEnum.MEMBER.getValue());
        return sessions.stream().map(session -> WebSocketFrameworkUtils.getLoginUserId(session)).collect(Collectors.toList());
    }
}
