...
import cn.metast.tuoke.module.system.mq.message.mail.MailSendMessage;
import cn.metast.tuoke.module.system.mq.producer.mail.MailProducer;
...
//                .setFrom("奥特曼 <ydym_test@163.com>")
    ...
    public void testDoSendMail_success() {
        try (final MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            // 准备参数
            MailSendMessage message = randomPojo(MailSendMessage.class, o -> o.setNickname("元圈"));
            // mock 方法（获得邮箱账号）
            ...
            mailUtilMock.when(() -> MailUtil.send(
                    argThat(mailAccount -> {
                        assertEquals("元圈 <7685@qq.com>", mailAccount.getFrom());
