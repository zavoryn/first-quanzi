...
import cn.metast.tuoke.module.system.controller.admin.socail.vo.client.SocialClientPageReqVO;
import cn.metast.tuoke.module.system.controller.admin.socail.vo.client.SocialClientSaveReqVO;
...
import cn.metast.tuoke.module.system.dal.redis.RedisKeyConstants;
...
public class SocialClientServiceImpl implements SocialClientService {
     ...
     * 3. developer：开发版
     */
    @Value("${tuoke.wxa-code.env-version:release}")
     ...
     * 3. formal：正式版
     */
    @Value("${tuoke.wxa-subscribe-message.miniprogram-state:formal}")
    ...
    public String getWxUnionid(Integer userType, String loginCode) {
        WxMaService service = getWxMaService(userType);
        try {
            WxMaJscode2SessionResult  wxMaJscode2SessionResult=service.jsCode2SessionInfo(loginCode);
            if(wxMaJscode2SessionResult==null) {
                return null;
            }
            return wxMaJscode2SessionResult.getUnionid();
        } catch (WxErrorException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public byte[] getWxaQrcode(SocialWxQrcodeReqDTO reqVO) {
        ...
        try {
           ...
           /* return service.getQrcodeService().createWxaCodeBytes(
                    reqVO.getPath(),
                    envVersion,
                    ObjUtil.defaultIfNull(reqVO.getWidth(), SocialWxQrcodeReqDTO.WIDTH),
                    ObjUtil.defaultIfNull(reqVO.getAutoColor(), SocialWxQrcodeReqDTO.AUTO_COLOR),
    ...
    public String getLinkUrl(String pageUrl,String title) {
        ...
        try {
            GenerateUrlLinkRequest request = GenerateUrlLinkRequest.builder()
                    ...
                    .envVersion("release")
                    .build();
            return service.getLinkService().generateUrlLink(request);
        } catch (WxErrorException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public String getShortUrl(String pageUrl, String title) {
        ...
        try {
            GenerateShortLinkRequest request = GenerateShortLinkRequest.builder()
                    ...
                    .isPermanent(false)
                    .build();
            return service.getLinkService().generateShortLink(request);
        } catch (WxErrorException e) {
            e.printStackTrace();
        }
        return null;
