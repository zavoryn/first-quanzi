...
import cn.metast.tuoke.module.system.controller.admin.auth.vo.AuthRegisterReqVO;
...
import cn.metast.tuoke.module.system.controller.admin.user.vo.user.UserImportExcelVO;
import cn.metast.tuoke.module.system.controller.admin.user.vo.user.UserImportRespVO;
...
import static cn.metast.tuoke.module.system.enums.LogRecordConstants.*;
...
//            success = SYSTEM_USER_UPDATE_SUCCESS)
    ...
    public PageResult<AdminUserDO> getUserPage(UserPageReqVO reqVO) {
        ...
        if (loginUserDeptId != null && loginUserDeptId == 115L) {
            // 超级管理员（部门ID为115），可以看所有数据
            deptIds = getDeptCondition(reqVO.getDeptId());
        } else {
            // 非超级管理员，只能看到本部门及子部门的用户
            Set<Long> loginUserDeptIds = getDeptCondition(loginUserDeptId);
            // 确保部门ID集合不为空
            if (CollUtil.isEmpty(loginUserDeptIds)) {
                loginUserDeptIds = new HashSet<>();
                loginUserDeptIds.add(loginUserDeptId);
            }
