...
import cn.metast.tuoke.framework.datapermission.core.annotation.DataPermission;
...
import cn.metast.tuoke.module.bpm.controller.admin.task.vo.task.*;
import cn.metast.tuoke.module.bpm.convert.task.BpmTaskConvert;
import cn.metast.tuoke.module.bpm.dal.dataobject.definition.BpmFormDO;
import cn.metast.tuoke.module.bpm.dal.dataobject.definition.BpmProcessDefinitionInfoDO;
import cn.metast.tuoke.module.bpm.enums.definition.*;
import cn.metast.tuoke.module.bpm.enums.task.BpmCommentTypeEnum;
import cn.metast.tuoke.module.bpm.enums.task.BpmReasonEnum;
import cn.metast.tuoke.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import cn.metast.tuoke.module.bpm.service.definition.BpmFormService;
import cn.metast.tuoke.module.bpm.service.definition.BpmModelService;
...
import cn.metast.tuoke.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
...
import static cn.metast.tuoke.module.bpm.framework.flowable.core.enums.BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_RETURN_FLAG;
import static cn.metast.tuoke.module.bpm.framework.flowable.core.enums.BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_SKIP_START_USER_NODE;
import static cn.metast.tuoke.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 tuoke-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void moveTaskToEnd(String processInstanceId, String reason) {
        ...
        // TODO 元圈：目前发现并行分支情况下，会存在这个情况，后续看看有没更好的方案；
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/tuoke-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
    ...
    public void processTaskAssigned(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCompletion(int transactionStatus) {
                ...
                // 自动去重，通过自动审批的方式 TODO @元圈 驳回的情况得考虑一下；@lesan：驳回后，又自动审批么？
                ...
                // TODO 元圈：【优化】未来有没更好的判断方式？！另外，还要考虑清理机制。就是说，下次处理了之后，就移除这个标识
