package cn.metast.tuoke.module.bpm.framework.flowable.core.util;
...
import cn.metast.tuoke.module.bpm.controller.admin.definition.vo.model.simple.BpmSimpleModelNodeVO.ConditionGroups;
...
import cn.metast.tuoke.module.bpm.framework.flowable.core.listener.BpmCopyTaskDelegate;
import cn.metast.tuoke.module.bpm.framework.flowable.core.listener.BpmTriggerTaskDelegate;
import cn.metast.tuoke.module.bpm.service.task.listener.BpmCallActivityListener;
import cn.metast.tuoke.module.bpm.service.task.listener.BpmUserTaskListener;
...
import static cn.metast.tuoke.module.bpm.framework.flowable.core.enums.BpmnModelConstants.*;
...
public class SimpleModelUtils {
    ...
    private static void traverseBranchNodeToBuildSequenceFlow(Process process, BpmSimpleModelNodeVO node, String targetNodeId) {
        ...
        // TODO @元圈 路由分支没有conditionNodes 这里注释会影响吗？@jason：一起帮忙瞅瞅！
//        Assert.notEmpty(conditionNodes, "分支节点的条件节点不能为空");
    ...
    private static SequenceFlow buildBpmnSequenceFlow(String sourceId, String targetId,
        ...
        // TODO @jason：如果 sequenceFlowId 不存在的时候，是不是要生成一个默认的 sequenceFlowId？ @元圈： 貌似不需要,Flowable 会默认生成；TODO @jason：建议还是搞一个，主要是后续好排查问题。
        // TODO @jason：如果 name 不存在的时候，是不是要生成一个默认的 name？ @元圈： 不需要生成默认的吧？ 这个会在流程图展示的， 一般用户填写的。不好生成默认的吧；TODO @jason：建议还是搞一个，主要是后续好排查问题。
    ...
    private static class StartUserNodeConvert implements NodeConvert {
        ...
        public UserTask convert(BpmSimpleModelNodeVO node) {
            ...
            // 使用自动通过策略
            // TODO @元圈 复用了SKIP， 是否需要新加一个策略；TODO @元圈：【回复】是不是应该类似飞书，搞个草稿状态。待定；还有一种策略，不标记自动通过，而是首次发起后，第一个节点，自动通过；
