...
import cn.metast.tuoke.module.infra.framework.file.core.client.FileClientConfig;
import cn.metast.tuoke.module.infra.framework.file.core.client.FileClientFactory;
import cn.metast.tuoke.module.infra.framework.file.core.client.local.LocalFileClient;
import cn.metast.tuoke.module.infra.framework.file.core.client.local.LocalFileClientConfig;
import cn.metast.tuoke.module.infra.framework.file.core.enums.FileStorageEnum;
import cn.metast.tuoke.framework.test.core.ut.BaseDbUnitTest;
import cn.metast.tuoke.module.infra.controller.admin.file.vo.config.FileConfigPageReqVO;
import cn.metast.tuoke.module.infra.controller.admin.file.vo.config.FileConfigSaveReqVO;
import cn.metast.tuoke.module.infra.dal.dataobject.file.FileConfigDO;
import cn.metast.tuoke.module.infra.dal.mysql.file.FileConfigMapper;
import lombok.Data;
...
import static cn.metast.tuoke.framework.test.core.util.RandomUtils.randomLongId;
import static cn.metast.tuoke.framework.test.core.util.RandomUtils.randomPojo;
import static cn.metast.tuoke.module.infra.enums.ErrorCodeConstants.FILE_CONFIG_DELETE_FAIL_MASTER;
import static cn.metast.tuoke.module.infra.enums.ErrorCodeConstants.FILE_CONFIG_NOT_EXISTS;
...
public class FileConfigServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateFileConfig_success() {
        ...
        Map<String, Object> config = MapUtil.<String, Object>builder().put("basePath", "/yunai")
                .put("domain", "https://www.metast.cn").build();
        ...
        assertEquals("https://www.metast.cn", ((LocalFileClientConfig) fileConfig.getConfig()).getDomain());
        // 验证 cache
    ...
    public void testUpdateFileConfig_success() {
        ...
        FileConfigDO dbFileConfig = randomPojo(FileConfigDO.class, o -> o.setStorage(FileStorageEnum.LOCAL.getStorage())
                .setConfig(new LocalFileClientConfig().setBasePath("/yunai").setDomain("https://www.metast.cn")));
        ...
        FileConfigSaveReqVO reqVO = randomPojo(FileConfigSaveReqVO.class, o -> {
            ...
            Map<String, Object> config = MapUtil.<String, Object>builder().put("basePath", "/yunai2")
                    .put("domain", "https://doc.metast.cn").build();
            o.setConfig(config);
        });
        ...
        assertEquals("https://doc.metast.cn", ((LocalFileClientConfig) fileConfig.getConfig()).getDomain());
        // 验证 cache
    ...
    public void testGetFileConfigPage() {
        // mock 数据
        FileConfigDO dbFileConfig = randomFileConfigDO().setName("元圈")
    ...
    public void testFileConfig() throws Exception {
        ...
        when(fileClient.upload(any(), any(), any())).thenReturn("https://www.metast.cn");

        // 调用，并断言
        assertEquals("https://www.metast.cn", fileConfigService.testFileConfig(id));
    }
