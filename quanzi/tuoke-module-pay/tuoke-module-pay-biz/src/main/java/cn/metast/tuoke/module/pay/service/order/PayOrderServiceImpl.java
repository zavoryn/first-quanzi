package cn.metast.tuoke.module.pay.service.order;
...
import cn.metast.tuoke.framework.common.util.date.LocalDateTimeUtils;
...
import cn.metast.tuoke.framework.pay.core.client.dto.order.PayOrderRespDTO;
import cn.metast.tuoke.framework.pay.core.client.dto.order.PayOrderUnifiedReqDTO;
import cn.metast.tuoke.framework.pay.core.client.impl.ylsw.YlswPayClientConfig;
import cn.metast.tuoke.framework.pay.core.enums.channel.PayChannelEnum;
import cn.metast.tuoke.framework.pay.core.enums.order.PayOrderStatusRespEnum;
...
import cn.metast.tuoke.module.crm.api.contract.ContractApi;
...
import cn.metast.tuoke.module.pay.controller.admin.order.vo.PayOrderFinanceReportExcelVO;
import cn.metast.tuoke.module.pay.controller.admin.order.vo.PayOrderPageReqVO;
import cn.metast.tuoke.module.pay.controller.admin.order.vo.PayOrderSubmitReqVO;
...
import cn.metast.tuoke.module.pay.dal.dataobject.order.PayOrderExtensionDO;
import cn.metast.tuoke.module.pay.dal.mysql.channel.PayChannelMapper;
import cn.metast.tuoke.module.pay.dal.mysql.order.PayOrderExtensionMapper;
import cn.metast.tuoke.module.pay.dal.mysql.order.PayOrderMapper;
import cn.metast.tuoke.module.pay.dal.redis.no.PayNoRedisDAO;
import cn.metast.tuoke.module.pay.enums.notify.PayNotifyTypeEnum;
import cn.metast.tuoke.module.pay.enums.order.PayOrderStatusEnum;
import cn.metast.tuoke.module.pay.framework.pay.config.PayProperties;
import cn.metast.tuoke.module.pay.service.app.PayAppService;
import cn.metast.tuoke.module.pay.service.channel.PayChannelService;
import cn.metast.tuoke.module.pay.service.notify.PayNotifyService;
...
import static cn.metast.tuoke.framework.common.util.json.JsonUtils.toJsonString;
...
public class PayOrderServiceImpl implements PayOrderService {
    ...
    public PageResult<PayOrderDO> getOrderPage(PayOrderPageReqVO pageReqVO) {
        ...
        // 获取部门对应的支付订单ID列表
        List<Long> payOrderIds = buyLogApi.getPayOrderIdsByDeptId(loginUserDeptId);
        ...
        // 空列表表示该部门没有配置圈子或没有订单，返回空结果
        if (payOrderIds.isEmpty()) {
            return PageResult.empty();
        }

        return orderMapper.selectPageByOrderIds(pageReqVO, payOrderIds);
    }
    ...
    public List<PayOrderDO> getOrderList(PayOrderExportReqVO exportReqVO) {
        ...
        return orderMapper.selectListByOrderIds(exportReqVO, payOrderIds);
    }
    ...
    public PayOrderSubmitRespVO submitOrder(PayOrderSubmitReqVO reqVO, String userIp) {
        ...
        // 1.2 判断是否有相同渠道的 银联商务 平台, 优先使用 银联商务 的支付平台渠道
        String channelCode = reqVO.getChannelCode();
        /*if(channelCode.indexOf("wx_") != -1 || channelCode.indexOf("alipay_") != -1){
            List<PayChannelDO> channels = channelService.getEnableChannelList(order.getAppId());
            List<String> channelCodes = channels.stream()
                    .map(PayChannelDO::getCode)
                    .collect(Collectors.toList());
            String channelCodeNew = channelCode.replace("wx_", "ylsw_").replace("alipay_", "ylsw_");
            boolean isInList = channelCodes.contains(channelCodeNew);
            if(isInList){
                channelCode = channelCodeNew;
            }
        }*/

        // 1.32 校验支付渠道是否有效
        PayChannelDO channel = validateChannelCanSubmit(order.getAppId(), channelCode);

        Map<String, Object> payClientToMap = channelService.getPayClientToMap(channel.getId());
        PayClient client = (PayClient) payClientToMap.get("client");
        String payConfig = payClientToMap.get("config").toString();
        ...
        if(channelCode.indexOf("ylsw_") != -1){
            unifiedOrderReqDTO.setOldPayType(reqVO.getChannelCode());
            unifiedOrderReqDTO.setYlswPayUrl(getYlswPayUrl(reqVO.getChannelCode()));
            PayClientConfig payClientConfig = getYlswPayClientConfig(channel);
            unifiedOrderReqDTO.setYlswConfig( (YlswPayClientConfig) payClientConfig);
        }
        ...
        if (unifiedOrderResp != null) {
            try {
                ...
                // 兼容 https://gitee.com/zhijiantianya/tuoke-cloud/issues/I8SM9H 场景
        ...
        PayOrderSubmitRespVO payOrderSubmitConvert = PayOrderConvert.INSTANCE.convert(order, unifiedOrderResp);
        payOrderSubmitConvert.setMerchantOrderId(order.getMerchantOrderId());
        return payOrderSubmitConvert;
    }
    ...
    void validateOrderActuallyPaid(Long id) {
        ...
        orderExtensions.forEach(orderExtension -> {
            ...
            PayClient payClient = channelService.getPayClient(orderExtension.getChannelId(), orderExtension.getChannelConfig());
            if (payClient == null) {
    ...
    @Override
    public List<PayOrderFinanceReportExcelVO> getFinanceReportList(PayOrderExportReqVO exportReqVO) {
        // 部门数据隔离：根据当前登录用户的部门过滤
        Long loginUserDeptId = SecurityFrameworkUtils.getLoginUserDeptId();
        ...
        // 1. 查询订单列表
        List<PayOrderDO> list;
        if (payOrderIds == null) {
            // null 表示超级管理员，不过滤
            list = orderMapper.selectList(exportReqVO);
        } else if (payOrderIds.isEmpty()) {
            // 空列表表示该部门没有配置圈子或没有订单，返回空结果
            return new ArrayList<>();
        } else {
            list = orderMapper.selectListByOrderIds(exportReqVO, payOrderIds);
        }
        ...
        // 2. 获取用户ID列表
        Set<Long> userIds = convertSet(list, payOrder -> Long.valueOf(payOrder.getCreator()));
        ...
        // 4. 批量获取圈子成员信息（合同日期、软件版本）
        List<TopicMemberRespDTO> topicMembers = topicApi.getTopicMemberListByUserIds(new ArrayList<>(userIds));
        Map<Long, TopicMemberRespDTO> topicMemberMap = topicMembers.stream()
                .collect(Collectors.toMap(
                        TopicMemberRespDTO::getUserId,
                        member -> member,
        ...
        List<String> orderNos = list.stream()
                .map(PayOrderDO::getNo)
                .filter(no -> no != null && !no.isEmpty())
                .collect(Collectors.toList());
        Map<String, ContractRespDTO> contractMap = contractApi.getContractMapByNos(orderNos);

        // 6. 组装财务报表数据
        List<PayOrderFinanceReportExcelVO> excelList = new ArrayList<>();
        for (PayOrderDO order : list) {
            PayOrderFinanceReportExcelVO vo = new PayOrderFinanceReportExcelVO();
            Long userId = Long.valueOf(order.getCreator());

            // 到款日期（创建时间）
            vo.setPaymentDate(order.getCreateTime());

            // 客户姓名
            MemberUserRespDTO user = userMap.get(userId);
            if(user!=null) {
                vo.setCustomerName(StringUtils.isNotBlank(user.getName()) ? user.getName() : user.getNickname());
            }
            // 合同金额（支付金额）
            vo.setContractAmount(order.getPrice());
            ...
            if(order.getStatus()!=null) {
                ...
                vo.setPayStatus(statusCode);
            }
            //支付渠道
            String channelCode = "";
            if(StringUtils.isNotBlank(order.getChannelCode())) {
                if (Objects.equals(order.getChannelCode(), "wx_pub")) {
                    channelCode = "微信 JSAPI 支付";
                } else if (Objects.equals(order.getChannelCode(), "wx_lite")) {
                    channelCode = "微信小程序支付";
                } else if (Objects.equals(order.getChannelCode(), "wx_app")) {
                    channelCode = "微信 App 支付";
                } else if (Objects.equals(order.getChannelCode(), "wx_native")) {
                    channelCode = "微信 Native 支付";
                } else if (Objects.equals(order.getChannelCode(), "wx_wap")) {
                    channelCode = "微信 Wap 网站支付";
                } else if (Objects.equals(order.getChannelCode(), "wx_bar")) {
                    channelCode = "微信付款码支付";
                } else if (Objects.equals(order.getChannelCode(), "alipay_pc")) {
                    channelCode = "支付宝 PC 网站支付";
                } else if (Objects.equals(order.getChannelCode(), "alipay_wap")) {
                    channelCode = "支付宝 Wap 网站支付";
                } else if (Objects.equals(order.getChannelCode(), "alipay_app")) {
                    channelCode = "支付宝App 支付";
                } else if (Objects.equals(order.getChannelCode(), "alipay_qr")) {
                    channelCode = "支付宝扫码支付";
                } else if (Objects.equals(order.getChannelCode(), "alipay_bar")) {
                    channelCode = "支付宝条码支付";
                }
            ...
            // 商户单号
            vo.setMerchantOrderNo(order.getNo());

            // 获取圈子成员信息
            TopicMemberRespDTO topicMember = topicMemberMap.get(userId);
            if (topicMember != null) {
                vo.setContractStartDate(topicMember.getStartTime());
                vo.setContractEndDate(topicMember.getEndTime());
                vo.setSoftwareVersion(topicMember.getTopicName());
            } else {
                vo.setContractStartDate(null);
                vo.setContractEndDate(null);
                vo.setSoftwareVersion("");
            }

            // 是否续约客户：pay_order.no 关联 crm_contract.no，比较 total_price 和 price
            String isRenewal = "否";
            if (order.getNo() != null) {
                ContractRespDTO contract = contractMap.get(order.getNo());
                if (contract != null && contract.getTotalPrice() != null && order.getPrice() != null) {
                    BigDecimal payPriceYuan = BigDecimal.valueOf(order.getPrice()).divide(BigDecimal.valueOf(100));
                    if (payPriceYuan.compareTo(contract.getTotalPrice()) != 0) {
                        isRenewal = "是";
                    }
                }
            }
            vo.setIsRenewal(isRenewal);

            // 是否退款
            vo.setIsRefund(order.getRefundPrice() != null && order.getRefundPrice() > 0 ? "是" : "否");

            // 退款金额
            vo.setRefundAmount(order.getRefundPrice());
    ...
    private String getYlswPayUrl(String channelCode){
        String payUrl = "";
        String payUrl_test = "";
        if(channelCode.equals("wx_wap")){
            payUrl = "https://api-mop.chinaums.com/v1/netpay/wxpay/h5-pay";
            payUrl_test = "https://test-api-open.chinaums.com/v1/netpay/wxpay/h5-pay";
        }
        else if(channelCode.equals("wx_lite")){
            payUrl = "https://api-mop.chinaums.com/v1/netpay/wx/unified-order";
            payUrl_test = "https://test-api-open.chinaums.com/v1/netpay/wx/unified-order";
        }
        else if(channelCode.equals("wx_app")){
            payUrl = "https://api-mop.chinaums.com/v1/netpay/wx/unified-order";
            payUrl_test = "https://test-api-open.chinaums.com/v1/netpay/wx/unified-order";
        }
        else if(channelCode.equals("alipay_wap")){
            payUrl = "https://api-mop.chinaums.com/v1/netpay/trade/h5-pay";
            payUrl_test = "https://test-api-open.chinaums.com/v1/netpay/trade/h5-pay";
        }
//        else if(channelCode.equals("alipay_lite")){
//            payUrl = "https://api-mop.chinaums.com/v1/netpay/trade/create";
//            payUrl_test = "https://test-api-open.chinaums.com/v1/netpay/trade/create";
//        }
        else if(channelCode.equals("alipay_app")){
            payUrl = "https://api-mop.chinaums.com/v1/netpay/trade/precreate";
            payUrl_test = "https://test-api-open.chinaums.com/v1/netpay/trade/precreate";
        }
...
//        return payUrl;
    ...
    public PayClientConfig getYlswPayClientConfig(PayChannelDO channel) {
