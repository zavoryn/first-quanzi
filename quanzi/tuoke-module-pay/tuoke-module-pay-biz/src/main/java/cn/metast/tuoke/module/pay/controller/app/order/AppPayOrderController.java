package cn.metast.tuoke.module.pay.controller.app.order;

import cn.metast.tuoke.framework.common.pojo.CommonResult;
import cn.metast.tuoke.framework.common.util.object.BeanUtils;
import cn.metast.tuoke.framework.pay.core.enums.channel.PayChannelEnum;
import cn.metast.tuoke.module.pay.controller.admin.order.vo.PayOrderRespVO;
...
public class AppPayOrderController {
    ...
    public CommonResult<PayOrderRespVO> getOrderByPost(@RequestParam("id") Long id,
                                                 @RequestParam(value = "sync", required = false) Boolean sync) {
        PayOrderDO order = payOrderService.getOrder(id);
        // sync 仅在等待支付
        if (Boolean.TRUE.equals(sync) && PayOrderStatusEnum.isWaiting(order.getStatus())) {
