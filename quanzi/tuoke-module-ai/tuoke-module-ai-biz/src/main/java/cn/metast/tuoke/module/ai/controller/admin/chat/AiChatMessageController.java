package cn.metast.tuoke.module.ai.controller.admin.chat;
...
import cn.metast.tuoke.module.ai.dal.dataobject.knowledge.AiKnowledgeSegmentDO;
import cn.metast.tuoke.module.ai.dal.dataobject.model.AiChatRoleDO;
import cn.metast.tuoke.module.ai.service.chat.AiChatConversationService;
import cn.metast.tuoke.module.ai.service.chat.AiChatMessageService;
...
public class AiChatMessageController {
    ...
    @Operation(summary = "发送消息（段式）", description = "一次性返回，响应较慢 ，支持文件上传")
    @PostMapping("/send-file")
    public CommonResult<AiChatMessageSendRespVO> sendMessageWithFile(
            @RequestPart("sendReqVO") @Valid AiChatMessageSendReqVO sendReqVO,
    ...
    @Operation(summary = "发送消息（流式）- 支持文件上传", description = "流式返回，支持文件上传")
    @PostMapping(value = "/send-stream-file", produces = MediaType.TEXT_EVENT_STREAM_VALUE, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public Flux<CommonResult<AiChatMessageSendRespVO>> sendChatMessageStreamWithFile(
            @RequestPart("sendReqVO") @Valid AiChatMessageSendReqVO sendReqVO,
            @RequestPart("file") MultipartFile file
    ) {
        return chatMessageService.sendChatMessageStream(sendReqVO, getLoginUserId());
