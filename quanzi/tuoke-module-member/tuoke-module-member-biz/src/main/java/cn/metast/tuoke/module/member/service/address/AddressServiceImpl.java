...
public class AddressServiceImpl implements AddressService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int updateAddress_live(Long userId, AppAddressUpdateReqVO updateReqVO) {
        // 校验存在,校验是否能够操作
        MemberAddressDO addressDO = getAddress(userId, updateReqVO.getId());
        if (addressDO == null) {
            return -1;
        }
        ...
        // 更新
        MemberAddressDO updateObj = AddressConvert.INSTANCE.convert(updateReqVO);
        return memberAddressMapper.updateById(updateObj);
    }
    ...
    @Override
    public int deleteAddress_live(Long userId, Long id) {
        // 校验存在,校验是否能够操作
