...
public class KeFuConversationServiceImpl implements KeFuConversationService {
    ...
    public void updateConversationLastMessage(KeFuMessageDO kefuMessage) {
        ...
        conversationMapper.updateById(new KeFuConversationDO().setId(kefuMessage.getConversationId())
                ...
                //.setReceiverId(kefuMessage.getReceiverId()));
    ...
    @Override
    public KeFuConversationDO getOrCreateConversation(Long userId, Long receiverId,Long topicId) {
        KeFuConversationDO conversation = conversationMapper.selectOne(
                new LambdaQueryWrapper<KeFuConversationDO>()
...
//                        .eq(KeFuConversationDO::getReceiverId, receiverId)
                        .apply("( user_id = " + userId + " AND receiver_id = " + receiverId + ") OR ( user_id = " + receiverId + " AND receiver_id = " + userId + ")")
        );
        // 没有历史会话，则初始化一个新会话
        if (conversation == null) {
            conversation = new KeFuConversationDO().setUserId(userId).setReceiverId(receiverId).setLastMessageTime(LocalDateTime.now())
                    .setLastMessageContent(StrUtil.EMPTY).setLastMessageContentType(KeFuMessageContentTypeEnum.TEXT.getType())
                    .setAdminPinned(Boolean.FALSE).setUserDeleted(Boolean.FALSE).setAdminDeleted(Boolean.FALSE)
                    .setAdminUnreadMessageCount(0).setTopicId(topicId);
            conversationMapper.insert(conversation);
        }
        return conversation;
    }
    ...
    @Override
    public KeFuConversationDO getOrCreateConversationGroup(Long userId, Long receiverId) {
        KeFuConversationDO conversation = conversationMapper.selectOne(
                new LambdaQueryWrapper<KeFuConversationDO>()
                        .isNotNull(KeFuConversationDO::getReceiverId)
                        .apply("( user_id = " + userId + " AND receiver_id = " + receiverId + ") OR ( user_id = " + receiverId + " AND receiver_id = " + userId + ")")
        ...
        if (conversation == null) {
            conversation = new KeFuConversationDO().setUserId(userId).setReceiverId(receiverId).setLastMessageTime(LocalDateTime.now())
                    .setLastMessageContent(StrUtil.EMPTY).setLastMessageContentType(KeFuMessageContentTypeEnum.TEXT.getType())
                    .setAdminPinned(Boolean.FALSE).setUserDeleted(Boolean.FALSE).setAdminDeleted(Boolean.FALSE)
                    .setAdminUnreadMessageCount(0).setGroupStatus(1L);//组状态，屏蔽会话用
            conversationMapper.insert(conversation);
        }
        return conversation;
    }
    ...
    @Override
    public List<KeFuConversationDO> getConversationListByUserId(Long userId) {
        return conversationMapper.selectListByUserId(userId);
    }
    @Override
    public List<KeFuConversationDO> getKefuConversationList_admin() {
        return conversationMapper.selectConversationList_admin();
    }

    @Override
    public List<KeFuConversationDO> getKefuConversationList_web(Long receiverId) {
        return conversationMapper.selectConversationList_web(receiverId);
    }

    @Override
    public List<KeFuConversationDO> getKefuConversationListTopId(Long topicId,String nickName) {
        return conversationMapper.getKefuConversationListTopId(topicId,nickName);
    }
}
