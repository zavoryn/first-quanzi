...
import cn.metast.tuoke.module.product.api.sku.dto.ProductSkuUpdateStockReqDTO;
...
import cn.metast.tuoke.module.trade.controller.admin.base.member.user.MemberUserRespVO;
import cn.metast.tuoke.module.trade.controller.admin.base.product.property.ProductPropertyValueDetailRespVO;
...
import cn.metast.tuoke.module.trade.controller.app.order.vo.*;
...
import cn.metast.tuoke.module.trade.framework.delivery.core.client.dto.ExpressTrackRespDTO;
import cn.metast.tuoke.module.trade.framework.order.config.TradeOrderProperties;
import cn.metast.tuoke.module.trade.service.brokerage.bo.BrokerageAddReqBO;
...
import static cn.metast.tuoke.framework.common.util.collection.CollectionUtils.convertMultiMap;
import static cn.metast.tuoke.framework.common.util.date.LocalDateTimeUtils.addTime;
...
public interface TradeOrderConvert {
    ...
    default PayOrderCreateReqDTO convert(TradeOrderDO order, List<TradeOrderItemDO> orderItems,
        ...
        createReqDTO.setBody(subject); // TODO 元圈：临时写死
        // 订单相关字段
    ...
    default PageResult<TradeOrderPageItemRespVO> convertPage(PageResult<TradeOrderDO> pageResult,
        ...
        List<TradeOrderPageItemRespVO> orderVOs = CollectionUtils.convertList(pageResult.getList(), order -> {
            ...
            //购买数量
            //商品价格
            orderVO.setSpuNameEx(xOrderItems.get(0).getSpuName());
            orderVO.setCountEx(xOrderItems.get(0).getCount());
            String price=formatPrice(xOrderItems);
            orderVO.setPriceEx(price);
            orderVO.setNickNameEx(memberUserMap.get(orderVO.getUserId()).getNickname());
            return orderVO;
        });
        return new PageResult<>(orderVOs, pageResult.getTotal());
    }
    default String formatPrice(List<TradeOrderItemDO> xOrderItems) {
        if (xOrderItems == null || xOrderItems.isEmpty()) {
            return "0.00";
        }
        Integer price = xOrderItems.get(0).getPrice();
        if (price == null) {
            return "0.00";
        }
        BigDecimal priceInYuan = new BigDecimal(price).divide(new BigDecimal(100), 2, RoundingMode.HALF_UP);
        String formattedPrice = priceInYuan.toString();
