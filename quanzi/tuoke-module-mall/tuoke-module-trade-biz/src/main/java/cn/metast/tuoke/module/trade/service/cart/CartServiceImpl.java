package cn.metast.tuoke.module.trade.service.cart;
...
import cn.metast.tuoke.module.trade.controller.app.cart.vo.*;
import cn.metast.tuoke.module.trade.convert.cart.TradeCartConvert;
import cn.metast.tuoke.module.trade.dal.dataobject.cart.CartDO;
import cn.metast.tuoke.module.trade.dal.mysql.cart.CartMapper;
...
import static cn.metast.tuoke.module.trade.enums.ErrorCodeConstants.CARD_ITEM_NOT_FOUND;
import static java.util.Collections.emptyList;

/**
 * 购物车 Service 实现类
 *
 * // TODO 元圈：未来优化：购物车的价格计算，支持营销信息；目前不支持的原因，前端界面需要前端 pr 支持下；例如说：会员价格；
 *
 * @author metast.cn
 */
@Service
...
public class CartServiceImpl implements CartService {
    ...
    @Override
    public int addCart_live(Long userId, AppCartAddReqVO addReqVO) {
        // 查询 TradeCartDO
        CartDO cart = cartMapper.selectByUserIdAndSkuId(userId, addReqVO.getSkuId());
        // 校验 SKU
        Integer count = addReqVO.getCount();
        ProductSkuRespDTO sku = productSkuApi.getSku(addReqVO.getSkuId());
        if (sku == null) {
            // 商品不存在
            return -1;
        }
    ...
    @Override
    public int updateCartCount_live(Long userId, AppCartUpdateCountReqVO updateReqVO) {
        // 校验 TradeCartDO 存在
        CartDO cart = cartMapper.selectById(updateReqVO.getId(), userId);
        if (cart == null) {
            // 购物车项不存在
        ...
        ProductSkuRespDTO sku = productSkuApi.getSku(cart.getSkuId());
        if (sku == null) {
            // 商品不存在
            return -1;
        }
        if (updateReqVO.getCount() > sku.getStock()) {
            // 商品库存不足
            return -2;
        }

        // 更新数量
        return cartMapper.updateById(new CartDO().setId(cart.getId())
                .setCount(updateReqVO.getCount()));
    }
    ...
    @Override
    public int deleteCart_live(Long userId, Long id) {
        // 查询 TradeCartDO 列表
        CartDO cart = cartMapper.selectById(id, userId);
        if (cart == null) {
            return 1;
        }
    ...
    @Override
    public List<CartDO> getCartDoList(Long userId) {
        if (userId == null) {
            return Collections.emptyList();
        }
        List<CartDO> carts = cartMapper.selectListByUserId(userId);
